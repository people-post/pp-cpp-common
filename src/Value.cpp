  ...
  return std::visit(
      [](const auto &x) -> ValueWire {
        ...
        if constexpr (std::is_same_v<V, int64_t>) {
          return {ValueWire::TAG_I64, pp::binaryPack(x)};
        }
        if constexpr (std::is_same_v<V, uint64_t>) {
          return {ValueWire::TAG_U64, pp::binaryPack(x)};
        }
        if constexpr (std::is_same_v<V, bool>) {
          return {ValueWire::TAG_BOOL, pp::binaryPack(x)};
        }
        if constexpr (std::is_same_v<V, double>) {
          return {ValueWire::TAG_DOUBLE, pp::binaryPack(x)};
        }
        if constexpr (std::is_same_v<V, std::string>) {
          return {ValueWire::TAG_STRING, pp::binaryPack(x)};
        }
        if constexpr (std::is_same_v<V, ObjectPtr>) {
          if (!x) {
            return {ValueWire::TAG_NULL, std::string()};
          }
          return {ValueWire::TAG_OBJECT, pp::binaryPack(*x)};
        }
        if constexpr (std::is_same_v<V, ArrayPtr>) {
          if (!x) {
            return {ValueWire::TAG_NULL, std::string()};
...
bool wireToValue(const ValueWire &w, std::optional<Value> &out) {
  ...
  case ValueWire::TAG_I64: {
    auto r = pp::binaryUnpack<int64_t>(w.payload);
    if (!r.isOk()) {
      return false;
    }
    out = r.value();
  ...
  case ValueWire::TAG_U64: {
    auto r = pp::binaryUnpack<uint64_t>(w.payload);
    if (!r.isOk()) {
      return false;
    }
    out = r.value();
  ...
  case ValueWire::TAG_BOOL: {
    auto r = pp::binaryUnpack<bool>(w.payload);
    if (!r.isOk()) {
      return false;
    }
    out = r.value();
  ...
  case ValueWire::TAG_DOUBLE: {
    auto r = pp::binaryUnpack<double>(w.payload);
    if (!r.isOk()) {
      return false;
    }
    out = r.value();
  ...
  case ValueWire::TAG_STRING: {
    auto r = pp::binaryUnpack<std::string>(w.payload);
    if (!r.isOk()) {
      return false;
    }
  ...
  case ValueWire::TAG_OBJECT: {
    ...
    if (!w.payload.empty()) {
      auto r = pp::binaryUnpack<Object>(w.payload);
      if (!r.isOk()) {
        return false;
      }
